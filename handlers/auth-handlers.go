...
func (dbCfg *DBConfig) RefreshTokenHandler(w http.ResponseWriter, r *http.Request, JWTSecret string) {
	...
	newExpiresAt := time.Now().Add(60 * 24 * time.Hour).Unix()
	err = dbCfg.DB.StoreRefreshToken(refreshToken, userId, newExpiresAt)
	if err != nil {
		respondWithError(w, http.StatusInternalServerError, "Failed to store new refresh token")
		return
	}
